...
export class Calendar {
  ...
  static getEventsByDate(date: string, cb: (data: Calendar[]) => void) {
    const db = getDb();
    const sql = `SELECT * FROM events WHERE date='${date}';`;
    db.all(sql, (err, events: Calendar[]) => {
      if (err) {
        console.error(err);
        throw "Error occured";
      }
  ...
  static onDeleteEvent(eventId: string, date: string) {
  ...
  static updateGroupEventsWhenDelete(eventId: string, events: Calendar[]) {
    ...
    for (let i = 0; i <= updatedEvents.length - 1; i++) {
      db.run(
        sql,
        [updatedEvents[i].orderNumber, updatedEvents[i].eventId],
        (err) => {
          if (err) {
            console.error(err);
            throw "Error occured";
  ...
  static updateOrderNumberInOneDay(
    updatedEvents: Calendar[],
    cb: (val: boolean) => void
  ) {
    Calendar.getEventsByDate(updatedEvents[0].date, (events) => {
      ...
      for (let i = 0; i <= events.length - 1; i++) {
        const updatedEvent = updatedEvents.find(
          (e) => e.eventId === events[i].eventId
        );
        if (updatedEvent) {
          db.run(
            sql,
            [updatedEvent.orderNumber, updatedEvent.eventId],
            (err) => {
              if (err) {
                console.error(err);
                isSuccess = false;
  ...
  static updateDragDropDiffDays(
    ...
    const db = getDb();
    const dragged_update_sql =
      "UPDATE events SET orderNumber=? WHERE eventId=?";
    const dropped_update_sql =
      "UPDATE events SET date=?, orderNumber=? WHERE eventId=?";
    // firstly we update dragged day events
    Calendar.getEventsByDate(
      updatedData.draggedDay[0]?.date,
      (draggedevents) => {
        let isSuccess = true;
        if (draggedevents.length) {
          for (let i = 0; i <= draggedevents.length - 1; i++) {
            const updatedEvent = updatedData.draggedDay.find(
              (e) => e.eventId === draggedevents[i].eventId
            );
            if (updatedEvent) {
              db.run(
                dragged_update_sql,
  ...
  static update(updatedEvent: Calendar, cb?: () => void) {
    const db = getDb();
    const data = [
      updatedEvent.date,
      updatedEvent.title,
    ...
    const sql =
      "UPDATE events SET date=?, title=?, description=?, colors=?, orderNumber=? WHERE eventId=?";
    db.run(sql, data, (err) => {
      if (err) {
        console.error(err);
        throw "Error occured";
